...
export default function OnboardingScreen() {
  ...
  const buttonWidth = useSharedValue(width - 48); // starts full-width on step 0
  const arrowOpacity = useSharedValue(0); // arrow hidden on step 0
  const textOpacity = useSharedValue(1); // text visible on step 0
  ...
  useEffect(() => {
    ...
    if (isFirst || isLast) {
      ...
      buttonWidth.value = withSpring(60, {
        damping: 42,
        stiffness: 200,
      });
  ...
  const handleFinish = async () => {
    ...
    try {
      const payload = { height, weight, age, gender, activityLevel };
  ...
  // ---------------------------------------------------------------------------
  // Screens (Rendered inside the FlatList)
  // ---------------------------------------------------------------------------
        ...
        <View className="flex-row justify-end mt-4">
          <Animated.View style={[styles.footerButton, animatedButtonStyle]}>
            <TouchableOpacity
              ...
              className="bg-accent"
            >
              {/* Arrow icon — fades in on middle steps */}
              <Animated.View
              ...
              {/* Text — fades in on first and last steps */}
